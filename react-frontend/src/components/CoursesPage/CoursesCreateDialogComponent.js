    ...
    const onSave = async () => {
     ...
     useEffect(() => {
                    ...
                    client
                        ...
                        .then((res) => {
                            settopics(res.data);
                            settopics(res.data.map((e) => { return { name: e['topicstitle'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.log({ error });
    ...
    return (
        <Dialog header="Create" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            ...
            <div>
                ...
                <Dropdown value={_entity?.topics} optionLabel="name" optionValue="value" options={topicsOptions} onChange={(e) => setValByKey("topics", e.value)} />
            </div>
            <div>
